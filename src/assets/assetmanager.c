...
/*
map<const char*, GLuint> AssetManager::textures;

GLuint AssetManager::getTexture(const char *name, bool mipmapping, bool wrap, bool flipVertical)
{
    int width;
    int height;
    unsigned char *data;
	...
	if(name.length() > 0)
	{
		...
		else
		{
			...
			{
				fprintf(stderr, "AssetManager::getTexture could not load '%s'",
				result = 0;
			}
		}
    }
